...
export const SHOP_ITEMS = {
    food_ration: {
        ...
        action: (G) => { G.player.hunger = Math.min(100, G.player.hunger + 40); log('Ate a food ration. +40 hunger.', 'success'); }
    },
    warm_coat: {
        name: 'Warm Coat', icon: '🧥', price: 35,
        description: 'Reduces cold loss 30%.',
        action: (G) => {
            if (G.player.hasCoat) { log('You already have a warm coat!', 'danger'); return false; }
            G.player.hasCoat = true;
            log('You put on the warm coat. Cold resistance +30%.', 'success');
        }
    },
    better_axe: {
        ...
        action: (G) => {
            if (G.player.steelAxe) { log('You already have a steel axe!', 'danger'); return false; }
            G.player.steelAxe = true;
            log('Upgraded to steel axe! Chop 2x wood.', 'success');
        }
    },
    arrows: {
        name: 'Arrows (10)', icon: '🏹', price: 10,
        description: 'Refills your arrows.',
        action: (G) => { G.player.arrows = Math.min(20, G.player.arrows + 10); log('+10 arrows.', 'success'); }
    },
    hot_soup: {
        ...
        action: (G) => {
            ...
            log('Hot soup warmed you up! +30 hunger, +20 warmth.', 'success');
        }
    },
    wood_bundle: {
        name: 'Wood Bundle', icon: '🪵', price: 6,
        description: '+5 firewood logs.',
        action: (G) => { G.player.wood += 5; log('+5 firewood logs.', 'success'); }
    },
};
